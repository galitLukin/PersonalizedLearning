...
function trainTree(X, Y, T, level, depth)
  ...
  treatments = ["A","B","C"]
  d=Dict([(i,count(x->x==i,Y)) for i in treatments])
...
for level in 1:4
    ...
    lnr = trainTree(train_X, train_Y, train_T, level, depth[level])
